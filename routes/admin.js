...
function normalizeBalanceBucket(bucket) {
  const value = String(bucket || "")
    .trim()
    .toLowerCase();
  if (value === "available") return "available";
  if (value === "locked" || value === "unavailable") return "locked";
  return null;
}
...
// ✅ Update user balance
router.patch("/users/:id/balance", verifyAdmin, updateUserBalance);
...
router.patch(
  "/users/:id/coins",
  verifyAdmin,
  require("../controller/adminUpdateCoin"),
);
...
router.post(
  "/users/:userId/orders/force-cancel",
  verifyAdmin,
  adminForceCancelUserOrders,
);
...
router.post(
  "/market-override/start",
  verifyAdmin,
  adminController.startMarketOverride,
);
router.post(
  "/market-override/stop",
  verifyAdmin,
  adminController.stopMarketOverride,
);
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const registersToday = await User.countDocuments({
      createdAt: { $gte: startOfToday },
    });
    const activeToday = await User.countDocuments({
      lastOnlineAt: { $gte: startOfToday },
    });

    const registersThisWeek = await User.countDocuments({
      createdAt: { $gte: startOfWeek },
    });
    const activeThisWeek = await User.countDocuments({
      lastOnlineAt: { $gte: startOfWeek },
    });

    const registersThisMonth = await User.countDocuments({
      createdAt: { $gte: startOfMonth },
    });
    const activeThisMonth = await User.countDocuments({
      lastOnlineAt: { $gte: startOfMonth },
    });
    ...
    for (const row of balanceRows) {
      ...
      if (!(asset in walletDistribution)) continue;

      walletDistribution[asset] +=
        Number(row.available || 0) + Number(row.locked || 0);
    }

    const walletList = Object.entries(walletDistribution).map(
      ([coin, value]) => ({
        coin,
        value,
      }),
...
router.post("/update-balance-availability", verifyAdmin, async (req, res) => {
  try {
    const { userId, coin, from, to, amount } = req.body;
    const asset = normalizeAsset(coin);
    const source = normalizeBalanceBucket(from);
    const target = normalizeBalanceBucket(to);
    ...
    const inc = { [source]: -amt, [target]: amt };
    const updated = await Balance.findOneAndUpdate(
      { userId, asset, [source]: { $gte: amt } },
      { $inc: inc },
      { new: true },
    );
...
router.patch("/reward-grants/:id/activate", verifyAdmin, async (req, res) => {
  try {
    ...
    if (!grant) {
      return res
        .status(404)
        .json({ message: "Grant not found or not in draft status" });
    }
...
router.patch("/reward-grants/:id/cancel", verifyAdmin, async (req, res) => {
  try {
    ...
    if (!grant) {
      return res
        .status(404)
        .json({ message: "Grant not found or cannot be cancelled" });
    }
...
// Authenticated account summary using Balance as the source of truth.
...
router.patch(
  "/users/:id/reset-withdrawal-pin-lock",
  verifyAdmin,
  async (req, res) => {
    try {
      const user = await User.findById(req.params.id);
